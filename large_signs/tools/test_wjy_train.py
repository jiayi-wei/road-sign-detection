...
def save_detections(image_name, dets):
	...
	im_file = os.path.join('../lib/datasets/test_/im',image_name+'.jpg')
	im = cv2.imread(im_file)
	outpath1 = os.path.join('../lib/datasets/test_/res',image_name+'.txt')
...
if __name__ == '__main__':
	...
	caffemodel = os.path.join(cfg.ROOT_DIR, 'output','faster_rcnn_end2end','hs','vgg16_faster_rcnn_iter_80000_third.caffemodel')
	if args.cpu_mode:
		caffe.set_mode_cpu()
	else:
		caffe.set_mode_gpu()
	...
	with open('/home/dl215/workspace/wei/detection_biaoshi2/py-faster-rcnn/lib/datasets/test_/test.txt','r') as images:
		im_names=images.readlines()
	for im_name in im_names:
		print im_name
		print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
		print 'Demo for test/{}'.format(im_name)
		im_file = os.path.join('../lib', 'datasets','test_',im_name.strip()+'.jpg')
		print im_file
		im = cv2.imread(im_file)
		#for i in range(0,3):
			#seg = [[[0,0],[600,800]],[[499,0],[1099,800]],[[999,0],[1599,800]]]
			#imSeg = im[seg[i][0][1]:seg[i][1][1],seg[i][0][0]:seg[i][1][0]]
		det = detectBySeg(net,im)
		if len(det) == 0:
			print "no"
			continue
		dets = np.array(det)
